...
<!--          v-close-popup-->
<!--        />-->
        <div class="text-caption text-deep-purple" v-if="props.data0.mode === 'edit'">{{ nodeName }}</div>
      </q-card-section>
...
if (props.data0.mode === "add") {
  title.value = 'index.addShortcuts'
} else {
  title.value = 'index.editShortcuts'
}
...
const editShortcuts = () => {
  ...
  if (props.data0.mode === 'add') {
    if (isEmptyStr(props.data0.prevId) || isEmptyStr(formData.value.websiteName) || isEmptyStr(formData.value.website)) {
      return $q.notify({
        type: 'negative',
    ...
    formData.value.prevId = props.data0.prevId
    formData.value.pageNo = props.data0.pageNo
    console.log(formData.value)
    window.shortcuts.addShortcuts(JSON.stringify(formData.value)).then((result) => {
      if (result.success) {
        $q.notify({
          type: 'positive',
          position: clientConfig.quasar.notify.position,
  ...
  } else if (props.data0.mode === 'edit') {
    if (isEmptyStr(formData.value.id) || isEmptyStr(formData.value.websiteName) || isEmptyStr(formData.value.website)) {
      return $q.notify({
        type: 'negative',
        position: clientConfig.quasar.notify.position,
        message: t('verifyMessage.dataNotNull')
