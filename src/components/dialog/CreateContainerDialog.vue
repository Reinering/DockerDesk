...
const onCreate = () => {
  ...
  let cmd = ''
  if (composeFile.filePath === '') {
    cmd = `"cd /mnt/${firstLower(composeFile.folderPath).replace(':', '').replace(/\\/g, '/')}" && ${serviceCmd.value} up -d`
  } else {
    cmd = `"cd /mnt/${firstLower(composeFile.folderPath).replace(':', '').replace(/\\/g, '/')}" && ${serviceCmd.value} -f ${composeFile.filePath.name} up -d`
  }
